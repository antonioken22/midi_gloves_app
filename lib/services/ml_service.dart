...
class MLService with ChangeNotifier {
  ...
  Future<void> _loadSamples() async {
    ...
    // Clear potentially conflicting DTW data from memory if any
    if (encoded != null) {
      try {
        final List<dynamic> decoded = jsonDecode(encoded);
        // Basic validation to ensure we don't crash on old 2D arrays if they exist in this key
        if (decoded.isNotEmpty) {
          final firstFeatures = decoded[0]['features'];
          if (firstFeatures is List &&
              firstFeatures.isNotEmpty &&
              firstFeatures[0] is List) {
            // Detected 2D array (DTW data) in this key? Unlikely if we used different keys,
            // but good for safety.
            _samples = [];
            return;
          }
